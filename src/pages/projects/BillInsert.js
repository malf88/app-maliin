...
const BillInsert = (props) => {
  ...
  useEffect(() => {
    async function loadCreditCards() {
      let creditcardsPromise = await listCreditCards(account.id)
  ...
  const handleOpenBill = async (state) => {
    await setOpenBill(state)
  }
  const insertBill = async () => {
    ...
    await billInsert(account.id, payload)
      .then((response) => {
  ...
  return (
    <>
      ...
      >
        <DialogTitle>
          Inserir lançamento para o projeto <strong>{account.name}</strong>
          <IconButton
            aria-label="close"
            onClick={() => {
              handleClose()
